...
```
The cleaned merge data has `r nrow(merged_data)` rows and `r ncol(merged_data)` columns. The variables are `r names(merged_data)`.
...
The trend line shows potential gender differences in physical activity patterns, which suggests male and female have similar activity for all three education levels.
...
This plot shows the different usage patterns between casual and member riders, with casual riders generally taking longer, more leisure-oriented rides.

There is a noticeable reduction in ride durations for both rider types from 2020 to 2024, possibly due to changes in infrastructure, pricing, or user behavior.

The seasonality effect is apparent, with July (summer) seeing longer ride durations than January (winter), particularly for casual riders.
...
Casual riders take longer rides, particularly on classic bikes in July, likely for leisure.

Members have more uniform ride durations, with little variation between bike types and months. They are likely for frequent, shorter trips.

Electric bikes are used for shorter trips by casual riders, especially in the July, likely because of their efficiency and speed. 
