...
ggsave("activity_time_plot.png", plot = activity_plot, width = 8, height = 6)
```

    ## `geom_smooth()` using formula = 'y ~ x'

``` r
knitr::include_graphics("activity_time_plot.png")
```

<img src="activity_time_plot.png" width="90%" />

Higher education levels correlate with more sustained physical activity
throughout the day, especially in the evening.

The graph shows clear daily cycles of physical activity, with higher
levels in the morning and evening and a dip in the early afternoon.

The trend line shows potential gender differences in physical activity
patterns, which suggests male and female have similar activity for all
...
median_duration = citi_bike_data %>%
  group_by(weekdays, year, month, member_casual) %>%
...
``` r
ggplot(median_duration, aes(x = weekdays, y = median_duration, fill = member_casual)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(year ~ month) +
  theme_minimal() +
  labs(title = "Median Ride Duration by Day of the Week, Year, and Month",
       x = "Day of the Week", y = "Median Duration (minutes)") +
  scale_fill_manual(values = c("casual" = "coral", "member" = "skyblue")) +
...
This plot shows the different usage patterns between casual and member
riders, with casual riders generally taking longer, more
leisure-oriented rides.

There is a noticeable reduction in ride durations for both rider types
from 2020 to 2024, possibly due to changes in infrastructure, pricing,
or user behavior.

The seasonality effect is apparent, with July (summer) seeing longer
ride durations than January (winter), particularly for casual riders.
...
``` r
ggplot(citi_bike_2024, aes(x = month, y = median_duration, fill = rideable_type)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ member_casual) +
  theme_minimal() +
  labs(title = "Distribution of Ride Duration by Membership, Bike Type, and Month (2024)",
       x = "Membership Status", y = "Ride Duration (minutes)") +
  scale_fill_manual(values = c("classic_bike" = "skyblue", "electric_bike" = "coral")) +
  theme_minimal()
```

<img src="p8105_hw3_jc6422_files/figure-gfm/unnamed-chunk-15-1.png" width="90%" />

Casual riders take longer rides, particularly on classic bikes in July,
likely for leisure.

Members have more uniform ride durations, with little variation between
bike types and months. They are likely for frequent, shorter trips.

Electric bikes are used for shorter trips by casual riders, especially
in the July, likely because of their efficiency and speed.
